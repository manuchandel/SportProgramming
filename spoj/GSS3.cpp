...
void modify(int start,int end,int x,int y,int segIndex){
	int mid=start+(end-start)/2;
	if(start==end){
		tree[segIndex].leftsum=tree[segIndex].rightsum=tree[segIndex].midsum=tree[segIndex].total=tree[segIndex].best=y;
		return;
	}
	int lc=2*segIndex+1,rc=2*segIndex+2;//int lc=(segIndex<<1)+1,rc=(segIndex<<1)+2;
	if(x<=mid)
		modify(start,mid,x,y,lc);
	else modify(mid+1,end,x,y,rc);
	tree[segIndex].leftsum=max(tree[lc].leftsum,tree[lc].total+tree[rc].leftsum);
	tree[segIndex].rightsum=max(tree[rc].rightsum,tree[rc].total+tree[lc].rightsum);
	tree[segIndex].midsum=tree[rc].leftsum+tree[lc].rightsum;
	tree[segIndex].total=tree[lc].total+tree[rc].total;
